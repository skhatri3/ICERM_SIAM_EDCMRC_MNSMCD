function [u_beta] = RegStokeslets2D_gtovelocity_diff_ep(y,g,...
...
for k = 1:N 
    ...
    [~, ~, S1, S2, ~] = reg_fncs_withdoublet_diff_ep(ep1,ep2,R1,R2, blob_num); %Mx1
    ...
    u1(:)=u1(:)-beta(k)*norm1*(S1.*norm1+S2.*normxy.*XY1)*g1(k)+...
        -beta(k)*norm2*(S2.*normxy.*XY1)*g2(k);

    u2(:)=u2(:)-beta(k)*norm2*(S1.*norm2+S2.*normxy.*XY2)*g2(k)+...
        -beta(k)*norm1*(S2.*normxy.*XY2)*g1(k);    
