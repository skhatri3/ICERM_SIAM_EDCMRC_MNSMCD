...
[MRS_Permeable_errors] = calculate_errors(u_error,v_error,xx1,xx2,dx_g,dy_g, 'MRS - Permeable')
...
% u_error_L1 = ds_x*ds_y*sum(sum(abs(u_error)))
% v_error_L1 = ds_x*ds_y*sum(sum(abs(v_error)))
% u_error_L2 = sqrt(ds_x*ds_y*sum(sum(u_error.^2)))
% v_error_L2 = sqrt(ds_x*ds_y*sum(sum(v_error.^2)))
% 
% % Flow Rates along permeable section (unsure if this is correct)
% [u1_perm_exact,u2_perm_exact,p_perm_exact] = permeablechannelexact(y1b,y2b,Da);
% normals_perm = normals(idx,:);
% 
% FlowRate = dx_g*sum(dot(normals_perm, u_perm))
% FlowRate_exact = dx_g*sum(dot(normals_perm, [u1_perm_exact,u2_perm_exact]))
% 
