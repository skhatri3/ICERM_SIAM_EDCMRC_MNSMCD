...
figure;
   plot(Nvals(1:end), Routs,'o-', 'LineWidth', 3, 'MarkerSize', ...
       10, 'Color', colorlb)

%%

for j=1:length(Nvals)-1
    difftops(j)=abs(Rtops(j+1)-Rtops(j));
    diffouts(j)=abs(Routs(j+1)-Routs(j));
   ...
   figure;
   loglog(Nvals(1:end-1), diffouts(1:end),'o-', 'LineWidth', 3, 'MarkerSize', ...
       10, 'Color', colorlb)
hold on
% loglog(Nvals(1:end), d2v(1:end), 's-', 'LineWidth', 3,'MarkerSize', 10, 'Color', colordb);
% loglog(Nvals(1:end), d1v(1:end), 'd-', 'LineWidth', 3,'MarkerSize', 10,'Color', colorp);
