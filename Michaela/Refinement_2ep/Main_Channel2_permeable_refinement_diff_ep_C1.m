...
ErrorMatrix = zeros(length(Nvals),length(C1vals));
% Net Flow Error Plot
figure;
hold on;
set(gca, 'FontSize', 17);
xlabel('$C_1$', 'FontSize', 18,'Interpreter','latex')
...
for k = 1:length(Nvals)
    ...
    name = ['N = ' num2str(N)];
    loglog(C1vals, ErrorMatrix(k,:),'o-', 'LineWidth', 2.5, 'MarkerSize', ...
       10,'DisplayName',name)
