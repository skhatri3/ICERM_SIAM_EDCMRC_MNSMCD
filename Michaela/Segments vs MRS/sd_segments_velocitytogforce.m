function [g] = sd_segments_velocitytogforce(y,ub,normals,mu,segData,blob_num)
%
%                                WARNING!!!! 
% 
% This code is not producing correct results. There seems to be a bug,
% likely with the SD component, in this code.
%
...
for k = 1:Nseg
    ...
    beta_seg(:,k) = (beta_start(k)+beta_end(k))/2; % beta value of segment k
...
D11tmp = beta_seg.*(S11.*Norm1.*Norm1 + NdotXY.*S21.*XY1.*Norm1 + NdotXY.*S22.*V1Mat.*Norm1).*LMat;
D22tmp = beta_seg.*(S11.*Norm2.*Norm2 + NdotXY.*S21.*XY2.*Norm2 + NdotXY.*S22.*V2Mat.*Norm2).*LMat;
D12tmp = beta_seg.*(S11.*Norm1.*Norm2 + NdotXY.*S21.*XY1.*Norm2 + NdotXY.*S22.*V1Mat.*Norm2).*LMat;
D21tmp = beta_seg.*(S11.*Norm2.*Norm1 + NdotXY.*S21.*XY2.*Norm1 + NdotXY.*S22.*V2Mat.*Norm1).*LMat;


C11 = beta_seg.*((S10.*Norm1.*Norm1 + NdotXY.*S20.*XY1.*Norm1 + NdotXY.*S21.*V1Mat.*Norm1).*LMat) - D11tmp;
C22 = beta_seg.*((S10.*Norm2.*Norm2 + NdotXY.*S20.*XY2.*Norm2 + NdotXY.*S21.*V2Mat.*Norm2).*LMat) - D22tmp;
C12 = beta_seg.*((S10.*Norm1.*Norm2 + NdotXY.*S20.*XY1.*Norm2 + NdotXY.*S21.*V1Mat.*Norm2).*LMat) - D12tmp;
C21 = beta_seg.*((S10.*Norm2.*Norm1 + NdotXY.*S20.*XY2.*Norm1 + NdotXY.*S21.*V2Mat.*Norm1).*LMat) - D21tmp;
