function [uperm] = sd_segments_gforcetovelocity(x,y,gseg,normals,mu,segData,blob_num)
%
...
% Constant beta value for each segment
...
for k = 1:Nseg
    ...
    ep = segData(k,3);    
    [~, ~, ~, ~, ~, ~, S10, S11, S20, S21, S22] = seg_reg_fncs(x,y,ep,blob_num);
    
    % Matrix Blocks for segment solution
    D11 = (Norm1.*S11.*Norm1 + NdotXY.*S21.*XY1.*Norm1 + NdotXY.*S22.*V1.*Norm1);
    D22 = (Norm2.*S11.*Norm2 + NdotXY.*S21.*XY2.*Norm2 + NdotXY.*S22.*V2.*Norm2);
    D12 = (Norm1.*S11.*Norm2 + NdotXY.*S21.*XY1.*Norm2 + NdotXY.*S22.*V1.*Norm2);
    D21 = (Norm1.*S11.*Norm2 + NdotXY.*S21.*XY2.*Norm1 + NdotXY.*S22.*V2.*Norm1);

    C11 = (Norm1.*S10.*Norm1 + NdotXY.*S20.*XY1.*Norm1 + NdotXY.*S21.*V1.*Norm1) - D11;
    C22 = (Norm2.*S10.*Norm2 + NdotXY.*S20.*XY2.*Norm2 + NdotXY.*S21.*V2.*Norm2) - D22;
    C12 = (Norm1.*S10.*Norm2 + NdotXY.*S20.*XY1.*Norm2 + NdotXY.*S21.*V1.*Norm2) - D12;
    C21 = (Norm1.*S10.*Norm2 + NdotXY.*S20.*XY2.*Norm1 + NdotXY.*S21.*V2.*Norm1) - D21;
    
    %Calculate velocity due to gforces on current segment, add to total
    %velocity
    u1(:) = u1(:) - L*beta_seg(k)*(C11*g1_start(k) + C12*g2_start(k) + D11*g1_end(k) + D12*g2_end(k));
    u2(:) = u2(:) - L*beta_seg(k)*(C21*g1_start(k) + C22*g2_start(k) + D21*g1_end(k) + D22*g2_end(k));
end
