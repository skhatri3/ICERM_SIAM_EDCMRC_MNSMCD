function [u] = st_segments_forcetovelocity(x,y,f,mu,segData)
%
...
% [(2:Nseg)'; 1]. This function utilizes blob phi.
...
for k = 1:Nseg
    ...
    % Matrix blocks for u = Afj + Bfk
    B11 = (ep.^2.*T12 - Q1 + T12.*XY1.*XY1 + T22.*(XY1.*V1 + V1.*XY1) + T32.*V1.*V1);
    B22 = (ep.^2.*T12 - Q1 + T12.*XY2.*XY2 + T22.*(XY2.*V2 + V2.*XY2) + T32.*V2.*V2);
    B12 = (T12.*XY1.*XY2 + T22.*(XY1.*V2 + XY2.*V1)+ T32.*V1.*V2);
    
    A11 = (ep.^2.*T02 - Q0 + T02.*XY1.*XY1 + T12.*(XY1.*V1 +V1.*XY1) + T22.*V1.*V1) - B11;
    A22 = (ep.^2.*T02 - Q0 + T02.*XY2.*XY2 + T12.*(XY2.*V2 +V2.*XY2) + T22.*V2.*V2) - B22;
    A12 = (T02.*XY1.*XY2 + T12.*(XY1.*V2 +V1.*XY2) + T22.*V1.*V2) - B12;
