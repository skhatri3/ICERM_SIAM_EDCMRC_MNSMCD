function [H10, H11, H20, H21, H22, H23, S10, S11, S20, S21, S22, S12, S23] = seg_reg_fncs(x,y,ep,blob_num)
...
% S12 and S23 for the segment method with beta varying linearly
...
switch blob_num
    case 1 % The basic blob
         ...
         [Q0,Q1,T02,T04,T06,~,~,T12,T14,T16,~,~,T22,T24,T26,~,~,T32,T34,T36,~] = segment_int_terms(x,y,ep);
         H10 = (ep2.*T02 - Q0)/(4*pi);
         H11 = (ep2.*T12 - Q1)/(4*pi);
         H20 = (T02)/(4*pi);
         H21 = (T12)/(4*pi);
         ...
         S12 = (T22 + ep2.*T24)/(2*pi);
         S20 = -(4*ep2.*T06 + 2*T04)/(2*pi);
         S21 = -(4*ep2.*T16 + 2*T14)/(2*pi);
         S22 = -(4*ep2.*T26 + 2*T24)/(2*pi);
         S23 = -(4*ep2.*T36 + 2*T34)/(2*pi);
    case 2 % The fancy blob
        % psi = 2*ep^4*(r^4 - 10*ep^2*r^2 + 5*ep^4)/(Pi*(r^2 + ep^2)^5)
        [Q0,Q1,T02,T04,T06,T08,T010,T12,T14,T16,T18,T110,T22,T24,T26,T28,T210,T32,T34,T36,T310] = segment_int_terms(x,y,ep);
